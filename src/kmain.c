...
static void draw_logo() {
  // get frame buffer base and draw a square. note that qemu's 1024x768 mode is
  // 24 bits (found in PixelBitMask)
...
void kmain(EFI_MEMORY_DESCRIPTOR *mem_map, UINTN map_size, UINTN desc_size) {
  ...
  for (UINTN i = 0; i < map_size; i += desc_size) {
    EFI_MEMORY_DESCRIPTOR *curr_desc =
        (EFI_MEMORY_DESCRIPTOR *)((char *)mem_map + i);
  ...
  UINTN pic_mask = 1 << 9;
  if (*edx & pic_mask) printf("APIC supported!\n");
